...
var tag_file = document.getElementById("tag-file")
tag_file.addEventListener("change",(ev) => {
    ...
    fileRead.onload = () => {        
        document.querySelector("#previewImgTag").src = fileRead.result
    }
})
...
$("#blankPr-image-btn").on(
    "click",() => {
        $('#blankPr-file').click()
    }
)

var tag_file = document.getElementById("blankPr-file")
tag_file.addEventListener("change",(ev) => {
    var file = ev.target.files[0]
    console.log(file);
    ...
    fileRead.readAsDataURL(file)
    fileRead.onload = () => {        
        document.querySelector("#blankPr-img").src = fileRead.result
    }
})
